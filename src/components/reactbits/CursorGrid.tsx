...
/*
 * Adapted from React Bits (reactbits.dev) — CursorGrid.
 *
 * Changes from upstream:
 *  - accepts `children`, rendered above the canvas, so pointer events from
 *    content layered on top bubble to this container (upstream is a bare
 *    background canvas and would never see the cursor behind other elements).
 *  - default colour is the brand copper rather than upstream magenta.
...
const CursorGrid = ({
  ...
  // Upstream assigns this during render; that trips React's "no refs during
  // render" rule, so it is synced in an effect instead. Declared before the
  // canvas effect below so propsRef is populated before first paint logic runs.
  useEffect(() => {
    propsRef.current = {
      cellSize,
      color,
  ...
  return (
    <div ref={containerRef} className={`cursor-grid${className ? ` ${className}` : ""}`}>
      ...
      <div className="cursor-grid__content">{children}</div>
    </div>
  );
};
